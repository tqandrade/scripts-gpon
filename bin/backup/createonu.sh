...
echo -n "INFORME A VLAN OU PRESSIONE ENTER PARA USAR A VLAN $VLAN: "
read V;
[ "$V" != "" ] && VLAN=$V
...
if [[ "$MODEL" == "F660" ]];then
	MODEL="F660_1"
	MODE=""
	while [[ "$MODE" != "dhcp" && $MODE != "pppoe" ]];do
		echo -n "INFORME O MODO DE OPERACAO [pppoe ou dhcp]: "
		read MODE
	done
...
if [[ "$MODE" == "pppoe" ]];then
	...
	echo -n "DESEJA OUTRA CONTA DE TELEFONE [s/n]: "
	read CTA2
	if [ "$CTA2" == "s" ];then
		echo -n "INFORME A CONTA SIP: "
		read CONTASIP2
	...
	if [[ "$TEL" == "s" ]];then		
	  ...
	  snmpset -d -v2c -c $COMMUNITY $IP \
		...
		if [ "$CONTASIP2" != "" ];then
			...
			#DETERMINA O VOIP PROTOCOL
			  snmpset -v2c -c $COMMUNITY $IP \
			 	ZXGPON-ONTMGMT-MIB::zxGponVoIPConfigDataSignalProtocolUsed.$ROW.2 i 2
	
			  snmpset -d -v2c -c $COMMUNITY $IP \
				ZXGPON-ONTMGMT-MIB::zxGponSIPAgentConfigDataProxyServer.$ROW.2 s fs.voice.valenet.com.br \
				ZXGPON-ONTMGMT-MIB::zxGponSIPAgentConfigDataOutboundProxy.$ROW.2 s fs.voice.valenet.com.br \
				ZXGPON-ONTMGMT-MIB::zxGponSIPAgentConfigDataPrimaryDNS.$ROW.2 a 177.152.174.13 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPAgentConfigDataSecondaryDNS.$ROW.2 a 177.152.174.14 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPAgentConfigDataUDPTCPPort.$ROW.2 i 5060 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPAgentConfigDataHostId.$ROW.2 i 1 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPAgentConfigDataRegExpTime.$ROW.2 i 3600 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPAgentConfigDataReRegStartTime.$ROW.2 i 360 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPAgentConfigDataRegServer.$ROW.2 s fs.voice.valenet.com.br \
				ZXGPON-ONTMGMT-MIB::zxGponSIPAgentConfigDataValidatScheme.$ROW.2 i 1 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPAgentConfigDataEntryStatus.$ROW.2 i 4
			
			  snmpset -d -v2c -c $COMMUNITY $IP \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataAgentId.$ROW.2 i 1 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataUserPartAOR.$ROW.2 s $CONTASIP2 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataDisplayName.$ROW.2 s $CONTASIP2 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataUserName.$ROW.2 s $CONTASIP2 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataPassword.$ROW.2 s $SENHASIP2 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataVMailServerURI.$ROW.2 s '' \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataVMailValidateScheme.$ROW.2 i 1 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataVMailUserName.$ROW.2 s '' \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataVMailPassword.$ROW.2 s '' \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataVMailRealm.$ROW.2 s '' \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataVMailSubsciptExpTime.$ROW.2 i 3600 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataNetworkDialPlanPtr.$ROW.2 i 0 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataServiceProfilePtr.$ROW.2 i 0 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataFeatureCodePtr.$ROW.2 i 0 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataReleaseTimer.$ROW.2 i 10 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataROHTimer.$ROW.2 i 15 \
				ZXGPON-ONTMGMT-MIB::zxGponSIPUserDataEntryStatus.$ROW.2 i 4 
